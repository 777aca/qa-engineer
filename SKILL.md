...
**无论 Q1 选了哪一类任务，最终都必须产出一份「测试用例清单」作为基础交付物。** 其他产物（自动化脚本、Bug 报告、性能报告、探索 bug 汇总表）是在"用例清单"基础上的**追加**，不是替代。

- **默认产出路径**：`discuss/testing/test-cases.xlsx`（并可选生成 `.xmind`）
- **默认内容来源**：
  - Q1 = 用例设计 / 接口测试 / UI E2E / 性能 / App → 基于需求/接口/页面**正向设计**一批用例
  - Q1 = Bug 定位与报告 → 基于该 bug **反向补一批回归用例**（防止再犯）
  - Q1 = 主动探索测试 → 把探索过程中**验证过的检查点**转成用例（详见 `references/exploratory-testing.md` 第 9.1 节）
  - Q1 = 测试计划 / 方案 / 报告 → 清单作为方案里的"测试范围"附件
- **用户不需要用例时必须显式说"不要用例"或"只要 XXX"**，否则默认出用例
- 用例清单至少包含字段：`id / title / priority / module / precondition / steps / expected`
- 若 Q3 没勾 xlsx/xmind，默认 Markdown 表格呈现；Q3 勾了 xlsx 则调 `scripts/cases_to_xlsx.py`、勾了 xmind 则调 `scripts/cases_to_xmind.py`
...
- Q3 默认**同时产出两份**：
  1. **测试用例清单** → `discuss/testing/test-cases.xlsx`（把探索走过的检查点固化成可回归的用例）
  2. **Bug 汇总表** → `discuss/bugs/bug-report.xlsx`（探索发现的问题）
  另附一份对话内 Markdown 速览
- **必须先问 Q4 档位 + Q5 平台**，再问"测试环境可以做哪些操作"（能登录吗？有可复用账号吗？能点"删除 / 支付"类按钮吗？）
- 探索完成后，再主动询问是否需要把高优 bug 按 `bug-report.md` 模板展开成正式报告、或继续扩展成自动化脚本
...
本技能**不扫描**以下内容（无论任何档位、任何场景）：

- **代码风格 / lint 规范**：缩进、引号、分号、行宽、命名 camelCase/snake_case、文件/目录结构、import 顺序等
- **接口命名规范**：RESTful 路径命名、HTTP 动词使用、字段 camelCase vs snake_case、错误码命名风格、URL 复数/单数等
- **代码洁癖类问题**：变量名起得好不好、注释够不够、函数是不是太长、是不是该抽工具类

**只关心功能是否跑通、是否符合预期、是否会炸**。遇到"命名不规范但功能正确"的代码/接口，直接跳过，不写入 bug 清单。

L4 档的代码审查仍然保留，但只聚焦**会导致运行时错误或功能缺陷**的问题（空值解引用、useEffect 依赖漏项、事件泄漏、危险 HTML、灾难性回溯等），**不评价命名与风格**。

### 0.1 不擅自拓展、不擅自质疑需求

- **以用户给定的需求为准**。需求说测 A 就只测 A，不自作主张扩成"顺便把 B C D 一起测了"
- **不质疑合理需求**：用户说"只跑 L1 主流程"、"只测这一个按钮"、"只看登录页"，照做，不要追问"为什么不顺便测 xxx"、不要输出"建议也覆盖 xxx"的扩展项
- **不擅自升档**：用户选 L2 就跑 L2，不要"为了更保险"偷偷把 L3 的边界值/响应式也跑了
- **不擅自加平台**：用户选 PC 就只测 PC，不要"顺便"补一轮移动端
- **确实不合理时才提出**：只有在**执行这个需求会导致明显错误结果**时才质疑，例如：
  - 用户让在生产环境跑破坏性测试（数据删除/支付真实执行）
  - 用户让"测移动端"但给的 URL 是明确的 PC 后台管理系统
  - 用户给的验收标准自相矛盾（期望 A 同时又期望非 A）
- 质疑时**一句话说清风险并等回复**，不要长篇大论劝说，也不要先斩后奏
